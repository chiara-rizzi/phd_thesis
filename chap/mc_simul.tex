...
\item Decay of unstable particles.
\end{itemize}
...
\end{equation}
\noindent In this expression, the $k$ represents the number of final state particles produced in addition to $X$ and $\mathcal{M}_{X+k}^{(l)}$
the amplitude to produce the final state $X+k$ computed with $l$ virtual loops. The number of extra partons and the number of loops identify the 
