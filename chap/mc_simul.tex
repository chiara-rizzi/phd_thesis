...
the main \gls{mc} generators used in the \gls{atlas} Collaboration are described in Section \ref{sec:mcgen}, and Sections \ref{sec:detsim} and \ref{sec:datacorr} touch briefly on the topics of detector simulation and data-driven corrections.
...
\item \Gls{psh} and matching between \gls{psh} and \glspl{me}.
...
\end{equation}
\noindent In this expression, the $k$ represents the number of final state quarks and gluons produced in addition to $X$ (legs) and $\mathcal{M}_{X+k}^{(l)}$
...
\begin{description}
\item[\PowhegBox] \cite{Nason:2004rx} In this framework it is possible to implement \gls{nlo} \glspl{me} computations, using the 5 flavour scheme.
