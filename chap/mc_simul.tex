...
\begin{description}
\item[\PY] \cite{Sjostrand:2006za,Sjostrand:2014zea} This is a general purpose generator that uses \gls{lo} matrix elements for $2\rightarrow n $ ($n \leq 3$) processes; 
it is capable to simulate both hard and soft interactions, including \gls{isr} and \gls{fsr} and multiple parton interactions.
It uses a \pt-ordered parton shower and the Lund string hadronization model.
...
\item[\Sherpa] \cite{Gleisberg:2008ta} The Sherpa event generator can provide multi-leg matrix elements both at \gls{lo} and at \gls{nlo}. 
...
\begin{description}
\item[\PowhegBox] \cite{Nason:2004rx}
\item[\aNLO] \cite{Alwall:2014hca}
\end{description}
