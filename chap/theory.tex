...
\lagr_{\rm FI} \,=\, -\kappa D .
\end{equation}
Whith This mechanism it is however difficult to attribute properly masses to all the MSSM particles.
%This type of term is gauge-invariant only for abelian groups. 
%the hypercharge symmetry group $U(1)_Y$ is not eligible 

Another possibility is to assign a VEV to an F term \cite{ORaifeartaigh:1975nky}. Since in the MSSM there is not a good candidate to be a gauge singlet with a non-vanishing VEV for an $F$-term, this implies the addition of at least one set of chiral supermultiplets, such that $F_i$ is non-trivial for at least one of them. A realization of this is possible for example with three chiral supermupliplets $\Phi_i$ and a superpotential 


When SUSY is treated as a local symmetry, it has to include also gravity, and the resulting theory is called \textit{supergravity}. The gravity mediator is the spin-2 graviton, and its superpartner the spin-$\frac{3}{2}$ \textit{gravitino} ($\tilde{G}$). The gravitino can be interpreted as the gauge field of SUSY, and it incorporates the goldstino degrees of freedom upon spontaneous SUSY breaking and is indicated with the same symbol, $\tilde{G}$. By dimensional arguments, we can understand that the gravitino mass ($m_{\frac{3}{2}}$) 
\begin{equation}
...
\label{msoft_PMSB}
\end{equation}
where $\langle F \rangle$ is the VEV of the SUSY-breaking $F$-term. This ratio is motivated by the observation that $m_{soft}$ should vanish both in the limit where SUSY is an exact symmetry, as well as in the limit where the Plank mass is very large and the interactions mediated by gravity are not anymore sizable. We refer to this scenario as Plank-scale-mediated-supersymmetry-breaking (PMSB). In PMSB models, the gravitino mass has the same order of magnitude as the mass of the other superpartners (as we can notice by comparing Eq.\ref{mgrav} and Eq. \ref{msoft_PMSB}), and it can not therefore be particularly light.

\paragraph{GMSB}

In the case of gauge-mediated-supersymmetry-breaking (GMSB) the interactions mediating the connection with the MSSM are the same gauge interactions of the MSSM itself.
