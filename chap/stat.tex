...
A second step consists in deriving an upper limit on the signal strength $\mu_{\rm SIG}$, which means finding by how much the 
...
The numerical upper limit values resulting from these \gls{cls} scans are reported in Table \ref{tab:stat:exampleUL}.
...
In this section we show example how combining different regions can increase the sensitivity. 
For this we use the same signal sample as in the previous examples. Just, in this case we divide the \gls{sr} into 
two mutually exclusive regions by introducing another discriminating variable, the number of leptons. We define SR1L with the 
same requirements as the original SR but in addition the requirement of at least one lepton, and the complementary SR0L with instead
a lepton veto. 
