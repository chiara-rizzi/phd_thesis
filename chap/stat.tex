...
\subsection{Advantages of the control regions}
...
\noindent While the pre-fit uncertainty on the \gls{vr} is large, the variation of the transfer factor is smaller:
\begin{equation}
\begin{split}
TF^{nominal}_{VR} &= \frac{62.9}{279.1} = 0.225 \; ,\\
TF^{up}_{VR} &= \frac{83.2}{360.3} = 0.231 \; , \\
TF^{down}_{VR} &= \frac{42.5}{197.8} = 0.215 \; . \\
\end{split}
\end{equation}

\noindent This is because the normalization of the background in the \gls{cr} absorbs the normalization uncertainty, and the effect of the uncertainty on the \gls{vr} depends only on the change in shape of the distribution. This example was carried out on a \gls{vr}, but the same is true for the uncertainty on the \gls{sr}.
Figure \ref{fig:stat:VRunc} shows the reduction of the systematic uncertainty: after the fit in the \gls{cr}, the shaded band indicating the uncertainty on the background estimate is smaller. 
...
Agreement between \gls{mc} simulation and pseudo-data \subref{fig:stat:VRprefitSys} before and \subref{fig:stat:VRpostfitSys} after the fit in the \gls{cr}.
}
