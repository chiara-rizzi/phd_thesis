...
\begin{figure}[h]
\centering 
\subfigure[VR pre-fit]{\includegraphics[width=0.48\textwidth]{figures/stat/examples/upperlimit_cls_poi_Signal_Freq_CLs_grid_ts3.pdf}\label{fig:stat:CLscan:toys}}
\subfigure[VR post-fit]{\includegraphics[width=0.48\textwidth]{figures/stat/examples/upperlimit_cls_poi_Signal_Asym_CLs_grid_ts3.pdf}\label{fig:stat:CLscan:asym}}
\caption{
...
The value of the upper limit for the signal strength can be extracted as the point where the expected line, the observed line and the uncertainty bands cross the horizontal line at 0.05, since this is the threshold we are using to set the limit.
The numerical values resulting from these \gls{cls} scans are reported in Table \ref{tab:stat:exampleUL}.
...
In this section we show with a simple example how combining different regions can increase the sensitivity. 
For this we use the same signal sample as in the previous examples. Just, in this case we divide the \gls{sr} into 
two mutually exclusive regions, named SR1 (with \meff between 3 and 3.5 TeV) and SR2 ($\meff > 3.5$ TeV); 
the two regions are shown in Figure \ref{fig:stat:example2}, which shows a zoomed version of the background and signal distributions of 
Figure \ref{fig:stat:example}.
