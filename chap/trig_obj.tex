...
for each jet it is possible to compute the \gls{jvf} the ratio of the sum of the track-momentum originating from the \gls{pv} to the total track momentum:

\begin{equation}
 \JVF({\rm jet}_{i},{\rm PV}_{j})=\frac{\sum_m \pt ({\rm track}_{m}^{\rm jet_i},{\rm PV}_j)}{\sum_n\sum_l  \pt ({\rm track}_{l}^{\rm jet_i},{\rm PV}_n)} \; .
 \label{eq:jvf}
\end{equation} 
