...
The missing transverse momentum ($\bar{p}_{\rm{T}}^{\rm{miss}}$) is defined as the negative vector sum of the \pt of all the reconstructed calibrated objects in the event, 
...
\begin{equation}
\bar{p}_{\rm{T}}^{\rm{miss}}=  - \sum \pt ^{e}  - \sum \pt ^{\gamma} - \sum \pt ^{\tau}    \nonumber \\
- \sum \pt ^{jets} - \sum \pt   ^{\mu} - \sum \pt  ^{soft-terms}   \;
\label{eq:etm} 
\end{equation}

\noindent The magnitude of the missing transverse momentum is the missing transverse energy (\met), and 
