...
Instead, in the cases of the Gtt model, other analyses 
...
Figure \ref{fig:summary_atlas_Gtt} shows the overlay of the limits obtained with the multi-$b$ search and with the two-same-charge-leptons search.
...
Figure \ref{fig:limits_higgsino_cms_comb} shows the combined exclusion contour: the expected sensitivity is up to about 580 GeV in the case \gls{br}$(\hino\rightarrow Z \tilde{G})=100$\% and 800 GeV for 
...
\item[Inclusive searches] Analyses that look for direct production of squarks (except 3$^{\rm{rd}}$ generation squarks)
