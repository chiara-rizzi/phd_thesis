...
Two dedicated discovery regions have optimized selections to maximize the discovery significance for the mass points with \mhino = 150 and 300 GeV:
\begin{description}
...
with very low signal contamination). $m_{2j}^\textrm{lead}$ and $m_{2j}^\textrm{subl}$ are used to define a \gls{cr} and two \glspl{vr}, both in the 4-tags and in the 2-tags samples; all these regions exclude the $X_{hh}^\textrm{SR}<1.6$ area, to be orthogonal to the \gls{sr}.
...
exclusion, respectively in the case $B(\hino\rightarrow h \tilde{G})=100$\% and in the \mhino-$B(\hino\rightarrow h \tilde{G})$ plane.
The results of the low-mass analysis are used below 300 GeV, while above it is the high mass analysis that provides the nominal result.
